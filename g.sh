...
# Use script-local venv if it exists, otherwise fallback to system python
PYTHON_BIN="${G_PYTHON_BIN:-$SCRIPT_DIR/.venv/bin/python3}"
...
Modes:
  ...
  default        running `g` with no arguments is equivalent to `g --audit .`
...
Search flags:
  ...
  stdin pipe     if no PATH is provided and stdin is a pipe, search stdin as a single text input
...
prune_chat_cache() {
  ...
  "$PYTHON_BIN" - "$dir" "$keep" "$max_mb" <<'PY' | xargs -0 -r rm -f 2>/dev/null || true
import os, sys
...
# ------------------------------------------------------------
# Dependency checks (do not auto-install)
# ------------------------------------------------------------
MISSING_DEPS=()
LOCAL_ALIAS_DIR="/tmp/g_local_bin_${UID:-0}"
...
install_if_missing() {
  ...
  # Support distro alternate command names (e.g. fdfind -> fd) without requiring root.
  if [[ -n "$alt_cmd" ]] && command -v "$alt_cmd" >/dev/null 2>&1; then
    mkdir -p "$LOCAL_ALIAS_DIR"
    ln -sf "$(command -v "$alt_cmd")" "$LOCAL_ALIAS_DIR/$cmd"
    PATH="$LOCAL_ALIAS_DIR:$PATH"
    if command -v "$cmd" >/dev/null 2>&1; then
      return 0
    fi
  fi

  MISSING_DEPS+=("$cmd (package: $pkg)")
}

report_missing_deps_and_exit() {
  ...
  echo "Install them and re-run g." >&2
  exit 127
}
...
# Default behavior: `g` with no args runs audit on current directory.
if [[ "$#" -eq 0 && "$AUDIT" -eq 0 ]]; then
  AUDIT=1
fi
...
if [[ "$CHAT_MODE" -eq 1 && -n "$CHAT_CACHE_DIR" ]]; then
...
need_cmd rg
[[ -x "$PYTHON_BIN" ]] || { echo "Error: python interpreter not found or not executable: $PYTHON_BIN" >&2; exit 127; }
need_cmd mktemp
need_cmd fd
...
DEP_CACHE="/tmp/g_dep_cache_$(id -u)"
if [[ -f "$DEP_CACHE" ]]; then
  source "$DEP_CACHE" 2>/dev/null || true
fi
if [[ -z "${HAVE_XLSX2CSV:-}" || -z "${HAVE_PPTX:-}" ]]; then
  HAVE_XLSX2CSV=0
  "$PYTHON_BIN" -c "import xlsx2csv" >/dev/null 2>&1 && HAVE_XLSX2CSV=1
  HAVE_PPTX=0
  "$PYTHON_BIN" -c "import pptx" >/dev/null 2>&1 && HAVE_PPTX=1
  echo "HAVE_XLSX2CSV=$HAVE_XLSX2CSV; HAVE_PPTX=$HAVE_PPTX" > "$DEP_CACHE" 2>/dev/null || true
fi
...
[[ $# -ge 1 ]] || { usage; exit 2; }
PATTERN="$1"; shift
if [[ "$FIXED_STRINGS" -eq 0 && "$PATTERN" == *'\x'* ]]; then
  PATTERN="$("$PYTHON_BIN" - "$PATTERN" <<'PY'
import re, sys
...
QUERY_KEY=""
 if [[ "$PAGE" != "0" ]]; then
  ...
  QUERY_KEY="$(
    "$PYTHON_BIN" - "$PATTERN" "$FIXED_STRINGS" "$CASE_SENSITIVE" "$CHAT_KEEP_TS" "$CHAT_NO_NORM" "$CHAT_NO_TOOLS" "$CHAT_PREFILTER" "$PAGE_SIZE" "$QUERY_CACHE_VERSION" "${PATHS[@]}" <<'PY'
...
PY
  ...
  if [[ "$PAGE" != "1" && -s "$QUERY_CACHE_DAT" && -s "$QUERY_CACHE_IDX" && -s "$QUERY_CACHE_META" ]]; then
    ...
    if "$PYTHON_BIN" - "$QUERY_CACHE_DAT" "$QUERY_CACHE_IDX" "$QUERY_CACHE_META" "$QUERY_KEY" "$QUERY_CACHE_VERSION" "$PAGE" "$PAGE_SIZE" <<'PY'
...
PY
    then
      ...
      start_us="${start_us:0:6}"
      while [[ ${#end_us} -lt 6 ]]; do end_us="${end_us}0"; done
      end_us="${end_us:0:6}"
      diff_s=$(( 10#$end_s - 10#$start_s ))
      diff_us=$(( 10#$end_us - 10#$start_us ))
...
tmp_dir="$(mktemp -d -t g_run.XXXXXX)"

tmp_fmt="$tmp_dir/g_fmt.py"
tmp_split="$tmp_dir/g_split.py"
tmp_xlsx="$tmp_dir/g_xlsx.py"
tmp_pptx="$tmp_dir/g_pptx.py"
tmp_doc="$tmp_dir/g_doc.sh"
tmp_chat="$tmp_dir/g_chat.py"
tmp_vsum="$tmp_dir/g_vsum.py"
tmp_failparse="$tmp_dir/g_failparse.py"

tmp_all="$tmp_dir/g_all.bin"
tmp_stdin="$tmp_dir/g_stdin.txt"
tmp_text="$tmp_dir/g_text.bin"
tmp_rich="$tmp_dir/g_rich.bin"
tmp_chat_list="$tmp_dir/g_chat_list.bin"
tmp_aichat_list="$tmp_dir/g_aichat_list.bin"
tmp_chat_all="$tmp_dir/g_chat_all.bin"
tmp_chat_text_list="$tmp_dir/g_chat_text_list.bin"
tmp_chat_bin_list="$tmp_dir/g_chat_bin_list.bin"
tmp_chat_prefilter="$tmp_dir/g_chat_prefilter.bin"
tmp_xlsx_list="$tmp_dir/g_xlsx_list.bin"
tmp_pptx_list="$tmp_dir/g_pptx_list.bin"
tmp_doc_list="$tmp_dir/g_doc_list.bin"

tmp_bad_rich="$tmp_dir/g_bad_rich.txt"
tmp_stats_json="$tmp_dir/g_stats.json"

tmp_rc2="$tmp_dir/g_rc2"
tmp_mc="$tmp_dir/g_mc"
tmp_shards_root="$tmp_dir/g_shards_root"
mkdir -p "$tmp_shards_root"

tmp_err_text="$tmp_dir/g_err_text.txt"
tmp_err_rich="$tmp_dir/g_err_rich.txt"
tmp_err_chat="$tmp_dir/g_err_chat.txt"
tmp_err_chat_prefilter="$tmp_dir/g_err_chat_prefilter.txt"
tmp_err_xlsx="$tmp_dir/g_err_xlsx.txt"
tmp_err_pptx="$tmp_dir/g_err_pptx.txt"
tmp_err_doc="$tmp_dir/g_err_doc.txt"
tmp_skip_rg="$tmp_dir/g_skip_rg.txt"
tmp_fail_out="$tmp_dir/g_fail_out.txt"
  ...
  if cached is None:
    ...
    if not lines and next_path.lower().endswith((".html", ".htm")):
      try:
        ...
        parser = FacebookHTMLParser(want)
...
cat >"$tmp_chat" <<'PY'
#!/home/lewis/Dev/deps/.venv/bin/python3
...
: >"$tmp_all"
if [[ "$USE_STDIN_INPUT" -eq 1 ]]; then
  cat >"$tmp_stdin"
  printf '%s\0' "$tmp_stdin" >>"$tmp_all"
else
  for p in "${PATHS[@]}"; do
    if [[ -f "$p" ]]; then
...
print(f"HAVE_XLSX2CSV={have_xlsx}")
print(f"HAVE_PPTX={have_pptx}")
PY
...
MIN_RICH_SIZE=128
if [[ "$CHAT_MODE" -eq 1 || "$NOCHAT" -eq 1 ]]; then
  split_out=""
  if [[ "$VERBOSE" -eq 1 ]]; then
    split_out="$("$PYTHON_BIN" -S "$tmp_split" "$tmp_all" "$tmp_text" "$tmp_rich" "$tmp_chat_list" "$tmp_aichat_list" "$tmp_xlsx_list" "$tmp_pptx_list" "$tmp_doc_list" "$tmp_bad_rich" "$tmp_stats_json" \
      "$EXT_FILTER_MODE" "$(IFS=,; echo "${FILTER_EXTS[*]}")" "$HAVE_RGA_PREPROC" "$MIN_RICH_SIZE" "$SEARCH_HIDDEN" "$CHAT_MODE" "$NOCHAT")"
  else
    split_out="$("$PYTHON_BIN" -S "$tmp_split" "$tmp_all" "$tmp_text" "$tmp_rich" "$tmp_chat_list" "$tmp_aichat_list" "$tmp_xlsx_list" "$tmp_pptx_list" "$tmp_doc_list" "$tmp_bad_rich" "$tmp_stats_json" \
      "$EXT_FILTER_MODE" "$(IFS=,; echo "${FILTER_EXTS[*]}")" "$HAVE_RGA_PREPROC" "$MIN_RICH_SIZE" "$SEARCH_HIDDEN" "$CHAT_MODE" "$NOCHAT" 2>/dev/null)" || true
  fi
  eval "$split_out"
else
  gawk -v RS='\0' -v mode="$EXT_FILTER_MODE" \
       -v filter_exts="$(IFS=,; echo "${FILTER_EXTS[*]}")" \
       -v have_rga_preproc="$HAVE_RGA_PREPROC" \
       -v min_rich_size="$MIN_RICH_SIZE" \
       -v search_hidden="$SEARCH_HIDDEN" \
       -v out_text="$tmp_text" \
       -v out_rich="$tmp_rich" \
       -v out_xlsx="$tmp_xlsx_list" \
       -v out_pptx="$tmp_pptx_list" \
       -v out_doc="$tmp_doc_list" \
       -v bad_rich_txt="$tmp_bad_rich" \
       -v stats_json="$tmp_stats_json" \
       '
@load "filefuncs"
...
function get_ext(p,    base, n, parts, ext) {
  n = split(p, parts, "/")
  base = parts[n]
  if (base ~ /^\.[^\.]+$/) return "(dotfile)"
  if (base ~ /\./) {
...
function is_hidden_path(p) {
  return (p ~ /(^|\/)\.[^\/.]/)
}

function allowed(ext) {
  if (mode == "all") return 1
  if (mode == "whitelist") return (ext in filter_exts_arr)
  if (mode == "blacklist") return !(ext in filter_exts_arr)
  return 1
}

function is_office_temp(p, ext,    base, n, parts) {
  n = split(p, parts, "/")
  base = parts[n]
  if (base ~ /^~\$/) {
    if (ext == "docx" || ext == "xlsx" || ext == "pptx" || ext == "xls" || ext == "ppt") {
      return 1
    }
  }
  return 0
...
BEGIN {
  n = split(filter_exts, fe_parts, ",")
  for (i = 1; i <= n; i++) {
    ext = tolower(trim(fe_parts[i]))
    if (ext != "") filter_exts_arr[ext] = 1
  }

  rich_exts_arr["pdf"] = 1; rich_exts_arr["docx"] = 1; rich_exts_arr["sqlite"] = 1;
  rich_exts_arr["sqlite3"] = 1; rich_exts_arr["db"] = 1; rich_exts_arr["db3"] = 1;
  
  xlsx_exts_arr["xlsx"] = 1; xlsx_exts_arr["xls"] = 1;
  pptx_exts_arr["pptx"] = 1; pptx_exts_arr["ppt"] = 1;
  doc_exts_arr["doc"] = 1;
}

{
  ...
  if (!(ext in ext_seen)) {
    ext_seen[ext] = 0; ext_attempted[ext] = 0; ext_hidden_skipped[ext] = 0;
    ext_blacklisted[ext] = 0; ext_excluded[ext] = 0; ext_skipped_own[ext] = 0;
    ext_skipped_bad_rich[ext] = 0; ext_skipped_encrypted[ext] = 0;
  }
  
  ext_seen[ext]++
  
  if ((search_hidden == 0) && is_hidden_path(p)) {
    ext_hidden_skipped[ext]++
    next
  }
  ...
  if (is_office_temp(p, ext)) {
    ext_skipped_own[ext]++
    ext_skipped_bad_rich[ext]++
    bad_count++
    bad_list[bad_count] = "skip-office-temp(~$): " p
    next
  }
  
  if (ext in xlsx_exts_arr) {
    printf "%s\0", p > out_xlsx
    ext_attempted[ext]++
    next
  }
  
  if (ext in pptx_exts_arr) {
    printf "%s\0", p > out_pptx
    ext_attempted[ext]++
    next
  }
  
  if (ext in doc_exts_arr) {
    printf "%s\0", p > out_doc
    ext_attempted[ext]++
    next
  }
  
  if (ext in rich_exts_arr) {
    if (have_rga_preproc == 0) {
      printf "%s\0", p > out_text
      ext_attempted[ext]++
      next
    }
    
    if (stat(p, arr) != 0 || arr["size"] < min_rich_size) {
      ext_skipped_own[ext]++
      ext_skipped_bad_rich[ext]++
      bad_count++
      bad_list[bad_count] = "skip-rich(small<" min_rich_size "): " p
      next
    }
    
    printf "%s\0", p > out_rich
    ext_attempted[ext]++
    next
  }
...
END {
  for (i = 1; i <= bad_count; i++) {
    print bad_list[i] > bad_rich_txt
  }
  
  close(out_text); close(out_rich); close(out_xlsx); close(out_pptx); close(out_doc); close(bad_rich_txt);
  
  json = "{\n"
  json = json "  \"mode\": \"" mode "\",\n"
  json = json "  \"have_rga_preproc\": " (have_rga_preproc ? "true" : "false") ",\n"
  json = json "  \"min_rich_size\": " min_rich_size ",\n"
  json = json "  \"search_hidden\": " (search_hidden ? "true" : "false") ",\n"
  json = json "  \"ext_stats\": {\n"
  ...
  for (e in ext_seen) {
    ...
    json = json "    \"" e "\": {\n"
    json = json "      \"seen\": " ext_seen[e] ",\n"
    json = json "      \"attempted\": " ext_attempted[e] ",\n"
    json = json "      \"hidden_skipped\": " ext_hidden_skipped[e] ",\n"
    json = json "      \"blacklisted\": " ext_blacklisted[e] ",\n"
    json = json "      \"excluded\": " ext_excluded[e] ",\n"
    json = json "      \"skipped_own\": " ext_skipped_own[e] ",\n"
    json = json "      \"skipped_bad_rich\": " ext_skipped_bad_rich[e] ",\n"
    json = json "      \"skipped_encrypted\": " ext_skipped_encrypted[e] "\n"
    json = json "    }"
  }
...
split_nul_to_shards() {
  ...
  "$PYTHON_BIN" - "$in_list" "$out_dir" "$k" <<'PY'
import os, sys
...
run_rg_json_parallel() {
  ...
  if [[ "$has_pre" -eq 0 ]]; then
    local st=0
    xargs -0 -a "$listfile" -r -- "${cmd[@]}" 2>>"$errfile" || st=$?
    if [[ "$st" -eq 1 ]]; then
      st=0
    fi
    if [[ "$st" -eq 141 && "$ALLOW_BROKEN_PIPE" -eq 1 ]]; then
      st=0
    fi
    return "$st"
  fi
...
if [[ "$CHAT_MODE" -eq 1 && "$CHAT_PREFILTER" -eq 1 && -s "$tmp_chat_all" ]]; then
  "$PYTHON_BIN" - "$tmp_chat_all" "$tmp_chat_text_list" "$tmp_chat_bin_list" <<'PY'
import os, sys
...
PY
  ...
  if [[ "$prefilter_failed" -eq 0 ]]; then
    "$PYTHON_BIN" - "$tmp_chat_prefilter" "$tmp_chat_bin_list" "$tmp_chat_all" <<'PY'
import sys
...
RG_XLSX=()
[[ "$HAVE_XLSX2CSV" -eq 1 ]] && RG_XLSX=("${RG_BASE[@]}" --pre "$PYTHON_BIN $tmp_xlsx" -- "$PATTERN")

RG_PPTX=()
[[ "$HAVE_PPTX" -eq 1 ]] && RG_PPTX=("${RG_BASE[@]}" --pre "$PYTHON_BIN $tmp_pptx" -- "$PATTERN")
...
(
  ...
  exit 0
) | "$PYTHON_BIN" -S "$tmp_fmt" "$BEFORE" "$AFTER" "$CTX_LINES" "$tmp_mc" "$MATCH_FILES_PERSIST" "$CHAT_MODE" "$COUNTS_ONLY" "$tmp_chat" "$tmp_skip_rg" "$BEFORE_TO_LINE_START" "$MERGE_MODE" "$PAGE" "$PAGE_SIZE" "$QUERY_CACHE_DAT" "$QUERY_CACHE_IDX" "$QUERY_CACHE_META" "$QUERY_KEY" "$QUERY_CACHE_VERSION"
...
for err in "$tmp_err_text" "$tmp_err_chat" "$tmp_err_rich" "$tmp_err_xlsx" "$tmp_err_pptx" "$tmp_err_doc"; do
  if [[ -s "$err" ]]; then
    "$PYTHON_BIN" "$tmp_failparse" "$err" "$tmp_fail_out" || true
...
PY
  ...
  "$PYTHON_BIN" "$tmp_vsum" "$tmp_stats_json" "$FAIL_PERSIST" "$tmp_skip_rg" 1>&2
fi
